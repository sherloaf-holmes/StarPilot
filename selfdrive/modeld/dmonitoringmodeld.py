...
class ModelState:
  def __init__(self, cam_w: int, cam_h: int):
    ...
    self.blob_misses = 0  # DIAG: how many times Tensor.from_blob was rebuilt
...
def main():
  ...
  # DIAG: periodic memory probe -> /data/dm_memtrace.log (remove once leak is found)
  def _rss_kb():
    try:
      with open("/proc/self/statm") as statm:
        return int(statm.read().split()[1]) * 4  # pages * 4KiB
    except Exception:
      return -1
  diag_frames = 0
  try:
  ...
  while True:
    ...
    diag_frames += 1
    if diag_frames % 600 == 0:
      with open("/data/dm_memtrace.log", "a") as diag_log:
        diag_log.write(f"frame={diag_frames} rss={_rss_kb()}kB cache={len(model._blob_cache)} "
                       f"from_blob_rebuilds={model.blob_misses}\n")
        if tracemalloc is not None:
          snap = tracemalloc.take_snapshot()
          if diag_prev_snap is not None:
            for stat in snap.compare_to(diag_prev_snap, "lineno")[:6]:
              diag_log.write(f"  {stat}\n")
          diag_prev_snap = snap
